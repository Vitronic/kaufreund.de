...
<div id="content">
  ...
  <div class="box">
    ...
    <div class="content">
	  <!--code start - modify -->
	  <!--add -->
      <div id="tabs" class="htabs"><a href="#tab-general"><?php echo $tab_general; ?></a><a href="#tab-data"><?php echo $tab_data; ?></a><?php if ($this->config->get('vendor_tab')) { ?><a href="#tab-vendor"><?php echo $tab_vendor; ?></a><?php } ?><a href="#tab-shipping"><?php echo $tab_shipping; ?></a><a href="#tab-links"><?php echo $tab_links; ?></a><a href="#tab-attribute"><?php echo $tab_attribute; ?></a><a href="#tab-option"><?php echo $tab_option; ?></a><a href="#tab-discount"><?php echo $tab_discount; ?></a><a href="#tab-special"><?php echo $tab_special; ?></a><a href="#tab-image"><?php echo $tab_image; ?></a><?php if ($this->config->get('vendor_reward_points')) { ?><a href="#tab-reward"><?php echo $tab_reward; ?></a><?php } ?><?php if ($this->config->get('vendor_desgin_tab')) { ?><a href="#tab-design"><?php echo $tab_design; ?></a><?php } ?></div>
      <!--code end-->
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        <div id="tab-general">
          ...
          <?php } ?>
		  <input type="hidden" name="product_name" id="product_name" size="100" value="<?php echo $product_name; ?>" />
			  ...
			  <table id="shipping" class="list">
				<thead>
				  <tr>
					...
					<td class="right"><?php echo $entry_shipping_cost; ?></td>
				...
				<tbody id="shipping-row<?php echo $shipping_row; ?>">
				  <tr>
					<td class="left"><select name="product_shipping[<?php echo $shipping_row; ?>][courier_id]">
						<?php foreach ($couriers as $courier) { ?>
					...
					<td class="right"><input type="text" name="product_shipping[<?php echo $shipping_row; ?>][shipping_rate]" value="<?php echo $product_shipping['shipping_rate']; ?>" /></td>
					<td class="right"><select name="product_shipping[<?php echo $shipping_row; ?>][geo_zone_id]">
					...
					<td class="left"><a onclick="$('#shipping-row<?php echo $shipping_row; ?>').remove();" class="button"><?php echo $button_remove; ?></a></td>
				  </tr>
				</tbody>
				<?php $shipping_row++; ?>
				<?php } ?>
				<tfoot>
				  <tr>
					<td colspan="3"></td>
					<td class="left"><a onclick="addShipping();" class="button"><?php echo $button_add_shipping; ?></a></td>
				  </tr>
				</tfoot>
			  </table>
			</div>
...
$('input[name=\'filter\']').autocomplete({
	...
	source: function(request, response) {
		$.ajax({
			url: 'index.php?route=catalog/pro2ven/filterAutocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request.term),
			dataType: 'json',
			success: function(json) {		
				response($.map(json, function(item) {
					return {
...
$('input[name=\'download\']').autocomplete({
	...
	source: function(request, response) {
		$.ajax({
			url: 'index.php?route=catalog/down2ven/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request.term),
			dataType: 'json',
			success: function(json) {		
				response($.map(json, function(item) {
					return {
...
function addShipping() {
	...
	html += '  <tr>'; 
	html += '    <td class="left"><select name="product_shipping[' + shipping_row + '][courier_id]">';
	<?php foreach ($couriers as $courier) { ?>
	html += '      <option value="<?php echo $courier['courier_id']; ?>"><?php echo $courier['courier_name']; ?></option>';
	<?php } ?>
	html += '    </select></td>';		
	html += '    <td class="right"><input type="text" name="product_shipping[' + shipping_row + '][shipping_rate]" value="" /></td>';
	html += '    <td class="right"><select name="product_shipping[' + shipping_row + '][geo_zone_id]">';
	<?php foreach ($geo_zones as $geo_zone) { ?>
	html += '      <option value="<?php echo $geo_zone['geo_zone_id']; ?>"><?php echo $geo_zone['name']; ?></option>';
	<?php } ?>
	html += '    </select></td>';	
	html += '    <td class="left"><a onclick="$(\'#shipping-row' + shipping_row + '\').remove();" class="button"><?php echo $button_remove; ?></a></td>';
	html += '  </tr>';
	html += '</tbody>';
				
	$('#shipping tfoot').before(html);
	$('#shipping-row' + shipping_row + ' .date').datepicker({dateFormat: 'yy-mm-dd'});
