...
class ControllerCommonHome extends Controller {   
	public function index() {
		...
		$data = array(
			...
			foreach ($results as $result) {
				...
				$this->data['orders'][] = array(
					...
					'total'      => $this->currency->format($result['total'] + $result['shipping_cost'] - $result['coupon_amount'] + ($this->config->get('tax_status') ? ($result['shipping_tax'] + $result['tax'] - $result['coupon_tax']) : 0), $result['currency_code'], $result['currency_value']),
					'action'     => $action
				);
			}
		...
		foreach ($results as $result) {
			...
			$order_status_result = $this->model_sale_order->getAllVendorOrderStatus($result['order_id']);
	...
	public function chart() {
		...
		switch ($range) {
			...
			if (!$this->user->getVP()) {
				...
				$query = $this->db->query("SELECT COUNT(DISTINCT (o.order_id)) AS total FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_product` op ON (o.order_id = op.order_id) WHERE o.order_status_id > '" . (int)$this->config->get('config_complete_status_id') . "' AND op.vendor_id = '" . (int)$this->user->getVP() . "' AND DATE(o.date_added) = '" . $this->db->escape($date) . "' GROUP BY DATE(o.date_added)");
			}
			...
			if (!$this->user->getVP()) {
				...
				$query = $this->db->query("SELECT COUNT(DISTINCT (o.order_id)) AS total FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_product` op ON (o.order_id = op.order_id) WHERE o.order_status_id > '" . (int)$this->config->get('config_complete_status_id') . "' AND op.vendor_id = '" . (int)$this->user->getVP() . "' AND YEAR(o.date_added) = '" . date('Y') . "' AND MONTH(o.date_added) = '" . $i . "' GROUP BY MONTH(o.date_added)");
			}
