	...
	private $error = array();
  	...
  	private function getForm() {
		...
		if (isset($this->request->post['duration'])) {
			...
			$this->data['duration'] = $commissions_info['duration'];
		} else {	
      		$this->data['duration'] = '';
    	}
		...
		$this->data['product_limits'] = $this->model_catalog_prolimit->getLimits();

		if (isset($this->request->post['product_limit_id'])) {
			$this->data['product_limit_id'] = $this->request->post['product_limit_id'];
		} elseif (isset($commissions_info)) {
			$this->data['product_limit_id'] = $commissions_info['product_limit_id'];
		} else {
			$this->data['product_limit_id'] = '1';
		}
