	...
	private $error = array();
  	...
  	public function info() {
		...
		if ($order_info) {
			...
			$this->data['tab_order_history'] = $this->language->get('tab_history');
			...
			foreach ($getOrderTotals as $order_totals) {
				...
				$this->data['totals'] = $this->currency->format($order_totals['total'] + $order_totals['shipping_cost'] - $order_totals['coupon_amount'] + ($this->config->get('tax_status') ? ($order_totals['shipping_tax'] + $order_totals['tax'] - $order_totals['coupon_tax']): 0), $order_info['currency_code'], $order_info['currency_value']);
				...
				if ($order_totals['coupon_amount'] != 0) {
					$this->data['coupon_title'] = $order_totals['coupon_title'];
					$this->data['coupon_amount'] = $this->currency->format($order_totals['coupon_amount'], $order_info['currency_code'], $order_info['currency_value']);
				} else {
					$this->data['coupon_amount'] = False;
				}
				
				if ($this->config->get('tax_status')) {
					$this->data['tax_title'] = $order_totals['tax_title'];
					$this->data['tax'] = $this->currency->format($order_totals['shipping_tax'] + $order_totals['tax'] - $order_totals['coupon_tax'], $order_info['currency_code'], $order_info['currency_value']);
				} else {
					$this->data['tax'] = False;
				}
			}
