...
class ControllerUserUser extends Controller {
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if ($this->config->get('bulk_products_activation')) {
				if ($this->request->post['status'] == '0') {
					$this->model_user_user->DisabledAllProducts($this->request->get['user_id']);
				} elseif ($this->request->post['status'] == '1') {
					$this->model_user_user->EnabledAllProducts($this->request->get['user_id']);
				}
			}
	...
	protected function getForm() {
		...
		$this->data['button_cancel'] = $this->language->get('button_cancel');
			...
			$this->data['txt_expired_date'] = $this->language->get('txt_expired_date');
		...
		if (isset($this->request->post['user_group_id'])) {
			...
			if (isset($this->request->post['user_date_start'])) {
				$this->data['user_date_start'] = $this->request->post['user_date_start'];
			} elseif (!empty($user_info['user_date_start'])) {
				$this->data['user_date_start'] = $user_info['user_date_start'];
			} else {
				$this->data['user_date_start'] = '';
			}
			
			if (isset($this->request->post['user_date_end'])) {
				$this->data['user_date_end'] = $this->request->post['user_date_end'];
			} elseif (!empty($user_info['user_date_end'])) {
				$this->data['user_date_end'] = $user_info['user_date_end'];
			} else {
				$this->data['user_date_end'] = '';
			}
