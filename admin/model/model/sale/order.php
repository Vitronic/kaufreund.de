	...
	public function getTotalOrderHistoriesByOrderStatusId($order_status_id) {
		...
		return $query->row['total'];
			...
			public function UpdateAllMultiVendorOrderStatus($order_id, $data) {
				...
				$this->db->query("UPDATE `" . DB_PREFIX . "order_product` SET order_status_id = '" . (int)$data['order_status_id'] . "' WHERE order_id = '" . $order_id . "'");
			}
						...
						foreach ($query->rows as $result) {						
							...
							if ($vname) {
								if (empty($status_text)) {
									$status_text = '<span class="help"><b>' . $vname . '</b> - ' . $status->row['name'] . '</span>';
								} else {
									$status_text .= '<span class="help"><b>' . $vname . '</b> - ' . $status->row['name'] . '</span>';
								}							
							}
