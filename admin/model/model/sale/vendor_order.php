...
class ModelSaleVendorOrder extends Model {
	...
	public function addOrderHistory($order_id, $data) {
		...
		if ($this->config->get('config_complete_status_id') == $data['order_status_id']) {
      	...
      	if ($data['notify']) {
			...
			if ($this->config->get('sales_order_allow_notification')) {
					$mail = new Mail();
	...
	public function UpdateMultiVendorOrderStatus($order_id, $data) {
		$this->db->query("UPDATE `" . DB_PREFIX . "order_status_vendor_update` SET order_status_id = '" . (int)$data['order_status_id'] . "', date_add = NOW() WHERE order_id = '" . $order_id . "' AND vendor_id = '" . (int)$this->user->getVP() . "'");
		$this->db->query("UPDATE `" . DB_PREFIX . "order_product` SET order_status_id = '" . (int)$data['order_status_id'] . "' WHERE order_id = '" . $order_id . "' AND vendor_id = '" . (int)$this->user->getVP() . "'");
	}
