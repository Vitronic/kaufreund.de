					...
					foreach ($vendor_unique as $vendor) {
						if ($vendor['vendor_id']) {
							...
							if (isset($coupon->row['amount']) > 0) {
								$template->data['coupon_title'] = $coupon->row['title'];
								$template->data['coupon'] = '-' . $this->currency->format($coupon->row['amount']);
							} else {
								$template->data['coupon'] = false;
							}
							...
							if ($this->config->get('tax_status') && ($vsubtotal->row['sum_product_tax'] != 0)) {
								...
								$template->data['tax'] = $this->currency->format($vsubtotal->row['sum_product_tax'] + (isset($shipcost->row['tax']) ? $shipcost->row['tax'] : '0') - (isset($coupon->row['tax']) ? $coupon->row['tax'] : '0'));
							} else {
								$template->data['tax'] = '0';
							}
				...
				if ($vendor_products){
					...
					foreach ($vendor_unique as $vendor) {
						if ($vendor['vendor_id']) {
							...
							if ($shipcost->rows) {
								if ($shipcost->row['cost']) {
									$total = $vsubtotal->row['sum_product_total'] + $shipcost->row['cost'] - (($coupon->row['amount']) ? $coupon->row['amount'] : 0) + ($this->config->get('tax_status') ? ($vsubtotal->row['sum_product_tax'] + $shipcost->row['tax'] -(($coupon->row['tax']) ? $coupon->row['tax'] : 0)): 0);
								} else {
									$total = $vsubtotal->row['sum_product_total'] - (($coupon->row['amount']) ? $coupon->row['amount'] : 0) + ($this->config->get('tax_status') ? ($vsubtotal->row['sum_product_tax'] - (($coupon->row['tax']) ? $coupon->row['tax'] : 0)): 0);
								}
